# flb_signv4.c and flb_signv4.h Documentation

## Overview

The `flb_signv4` module implements AWS Signature Version 4 (SigV4) authentication for HTTP requests. This implementation follows the official AWS SigV4 specification and is used to authenticate requests to AWS services.
...
Performs AWS Signature Version 4 signing of an HTTP request.

**Parameters:**
- `c`: HTTP client context
...
**Returns:**
- Normalized URI as SDS string
- `NULL` on error

## Implementation Details

The AWS SigV4 implementation follows the four-step process defined by AWS:

### Task 1: Create a Canonical Request

Constructs a canonical request by:
- Including the HTTP method
- Normalizing and encoding the URI
- Formatting query string parameters
- Preparing canonical headers
...
struct flb_http_client *client = flb_http_client(
    host, port, uri, method, body, body_len, headers, net);
...
flb_sds_t auth_header = flb_signv4_do(
    client, FLB_TRUE, FLB_TRUE, time(NULL),
    "us-east-1", "s3", S3_MODE_NONE, NULL, provider);

if (auth_header != NULL) {
    // Request is now signed and ready to send
    flb_http_client_do(client);
